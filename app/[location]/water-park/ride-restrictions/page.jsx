...
import { getRideRestrictions } from "@/lib/strapi/getRideRestrictions";
...
export async function generateMetadata({ params }) {
    ...
    const data = await getRideRestrictions(location);
    const city = loc.name || loc.displayName || location;

    return {
        title: `${data.seo.title} | Aqua Imagicaa ${city}`,
        description: data.seo.description,
        alternates: { canonical: `/${location}/water-park/ride-restrictions` },
    };
}

export default async function RideRestrictionsPage({ params }) {
    const { location } = await params;
    const loc = getLocation(location);
    if (!loc) notFound();
