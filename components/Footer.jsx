...
export function Footer({ location, navItems, locations = [] }) {
  const base = `/${location.slug}`;
  const links = navItems?.length ? navItems : NAV_LINKS;
  const columns = buildFooterColumns(links);

  // Find matching Strapi location for logo + address
  const strapi = locations.find((l) => l.slug === location.slug);
  const footerAddress = strapi?.contact?.address || strapi?.address || location.address?.line || '';
  const footerLogo = strapi?.logo;
    ...
    <footer className="footer-shell">
        ...
        <div className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr] gap-12 max-[1024px]:grid-cols-3 max-[720px]:grid-cols-2 max-[720px]:gap-8">
          <div className="max-[1024px]:col-span-3 max-[720px]:col-span-2">
            {footerLogo ? (
              <Image
                src={footerLogo}
                alt={`Aqua Imagicaa ${location.name}`}
                width={160}
                height={60}
            ...
            <h3 className="text-[32px] font-extrabold tracking-tight mt-5 mb-3">
              Aqua Imagicaa
            </h3>
        ...
        <div className="flex gap-4 mt-8">
          <a href="https://www.facebook.com/AquaImagicaa" target="_blank" rel="noopener noreferrer" className="text-white/75 hover:text-white">
            ...
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
              <path d="M22.675 0H1.325C.593 0 0 .593 0 1.326v21.348C0 23.407.593 24 1.325 24h11.495v-9.294H9.691v-3.622h3.129V8.413c0-3.1 1.893-4.788 4.659-4.788 1.325 0 2.463.099 2.794.143v3.24l-1.918.001c-1.504 0-1.795.715-1.795 1.763v2.313h3.587l-.467 3.622h-3.12V24h6.116C23.407 24 24 23.407 24 22.674V1.326C24 .593 23.407 0 22.675 0z" />
            </svg>
          </a>
      ...
      <div className="container-x mt-16 max-[720px]:mt-12">
        ...
        <div className="flex justify-between items-center mt-[72px] pt-8 border-t border-white/10 text-white/50 text-[13px] max-[720px]:flex-col max-[720px]:gap-3 max-[720px]:mt-12 max-[720px]:text-center">
          <div>© {new Date().getFullYear()} Aqua Imagicaa. All rights reserved.</div>
        </div>
      </div>
    </footer>
  );
