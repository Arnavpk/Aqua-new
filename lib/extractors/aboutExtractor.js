...
export function extractAboutStats(entry) {
    const sections = entry?.sections || [];
    const section = sections.find(
        (s) => s.__component === 'sections.about-stats'
    );
    if (!section?.stats_text) return null;

    // "20+,International rides,20000+ Sq. Ft.,Wavepool,2,Locations,..."
    // Split into pairs: [number, label, number, label, ...]
