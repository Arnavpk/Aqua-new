...
export function extractFeaturedSpotlight(strapiRides) {
    if (!strapiRides || strapiRides.length === 0) return null;
    ...
    return {
        slug: featured.slug,
        name: featured.name_emphasis
            ? featured.name.replace(featured.name_emphasis.replace(".", ""), "").trim()
            : featured.name,
        desc: featured.lede || featured.description,
