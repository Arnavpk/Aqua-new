...
export function extractOffers(strapiOffers) {
    ...
    return strapiOffers.map((o) => ({
        slug: o.slug,
        name: o.name,
        nameEm: o.name_emphasis || "",
        nameSuffix: o.name_suffix || "",
        tag: o.tag,
...
function firstMedia(field) {
    if (!field) return null;
    if (Array.isArray(field)) return field[0] || null;
    if (field.data && Array.isArray(field.data)) return field.data[0] || null;
    return field;
}
...
export function extractOfferDetail(strapiOffer) {
    ...
    return {
        ...
        bImage: getStrapiMedia(firstMedia(strapiOffer.b_image)),
        bImageMobile: getStrapiMedia(firstMedia(strapiOffer.b_mobile_image)),
        ...
        faq: {
            eyebrow: strapiOffer.faq_eyebrow || "Good to know",
            heading: strapiOffer.faq_heading || "Questions about this offer.",
            faqs: (strapiOffer.faqs || []).map((f) => ({
                q: f.question,
                a: f.answer,
                cat: f.category,
            })),
...
export function extractTermsContent(page) {
    if (!page?.sections) return null;
    const section = page.sections.find((s) => s.__component === 'sections.terms-content');
